...
const renderProjectView = async ({
  ...
  client.query.mockResolvedValueOnce({ data: { project: { tasks } }})
  const utils = render(<ProjectView match={{ params: { projectId } }} />)
...
describe('on wide screens', () => {
  ...
  it('displays a list of added tasks.', async () => {
    const { tasks, getAllByTestId } = await renderProjectView()
  ...
  it('dragging a task from one list to another changes its state.', async () => {
    ...
    const { getByTestId, getAllByTestId } = await renderProjectView({ tasks: [task] })
  ...
  it('using the add task modal creates a new task on the board.', async () => {
    ...
    const { tasks, getByTestId, getAllByTestId, debug } = await renderProjectView()
...
describe('on narrow screens', () => {
  ...
  it('displays only the added tasks after clicking the added button.', async () => {
    const { tasks, getByTestId, getAllByTestId } = await renderProjectView()
  ...
  it('using the move menu changes a task state.', async () => {
    ...
    const { getByTestId, debug } = await renderProjectView({ tasks: [task] })
  ...
  it('using the add task modal creates a new task on the board.', async () => {
    ...
    const { getByTestId } = await renderProjectView()
